...
2. **Strategy Fetching**: The system scans multiple sources for available trading opportunities using GraphQL endpoints and various data sources. The agent maintains a whitelist (primarily stablecoins, though subject to change) and filters opportunities based on this whitelist. For each strategy, the system calculates three critical metrics:
   ...
   - **Impermanent loss risk score**: Assesses potential losses from price divergence
    Note, the choice and calculation of selector metrics is subject to change based on opportunity type.
...
- Filters whitelisted assets based on recent price changes (removes assets with >5% price drops in 24 hours). For assets that have the **Stable** metadata tag AND as denominated by their stable asset, also supplied as a metadata entity.
