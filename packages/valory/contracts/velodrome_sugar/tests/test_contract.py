...
class TestPositions:
    ...
    def test_positions_calls_get_instance(self) -> None:
        """Test that positions calls get_instance with correct arguments.
    ...
    def test_positions_maps_all_16_fields_correctly(self) -> None:
        """Test that each of the 16 position fields is mapped to the correct index.
        ...
        mock_ledger_api.api.to_checksum_address.return_value = CHECKSUMMED_ACCOUNT
        mock_contract_instance = MagicMock()
        # Use distinct values for each field
        position_tuple = tuple(range(16))
        mock_contract_instance.functions.positions.return_value.call.return_value = [
            position_tuple
        ]
        ...
        assert pos["id"] == 0
        assert pos["lp"] == 1
        assert pos["liquidity"] == 2
        assert pos["staked"] == 3
        assert pos["amount0"] == 4
        assert pos["amount1"] == 5
        assert pos["staked0"] == 6
        assert pos["staked1"] == 7
        assert pos["unstaked_earned0"] == 8
        assert pos["unstaked_earned1"] == 9
        assert pos["emissions_earned"] == 10
        assert pos["tick_lower"] == 11
        assert pos["tick_upper"] == 12
        assert pos["sqrt_ratio_lower"] == 13
        assert pos["sqrt_ratio_upper"] == 14
        assert pos["alm"] == 15
