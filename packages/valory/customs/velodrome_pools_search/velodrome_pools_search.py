        ...
        "outputs": [
            {
                "components": [
                    ...
                    },
                    {
                        ...
                        "internalType": "struct RewardsSugar.TokenAmount[]",
                        "name": "fees",
                        "type": "tuple[]",
                    },
...
CACHE = {
    ...
    "token_symbols": {
        ...
        "timestamp": 0,
        "ttl": 86400,
    },  # 24 hours for token symbols
    "formatted_pools": {
        "data": {},
        "timestamp": 0,
        "ttl": 1800,
    },  # 30 minutes for formatted pools
}
...
def calculate_velodrome_il_risk_score_multi(
    ...
    if len(valid_token_ids) < 2:
        logger.warning(
            f"Not enough valid token IDs for pool {pool_id} on chain {chain}. Found: {valid_token_ids}"
        )
        return None
    ...
    try:
        ...
        if min_length < 2:
            logger.warning(
                f"Insufficient price data points for pool {pool_id} on chain {chain}. Min length: {min_length}"
            )
            return None
        ...
        for i, token_id in enumerate(valid_token_ids):
            debug_data["price_data"][token_id] = {
                ...
                "first_price": (
                    prices_data[i][0]
                    if i < len(prices_data) and len(prices_data[i]) > 0
                    else None
                ),
                "last_price": (
                    prices_data[i][-1]
        ...
        debug_data["calculations"]["initial_prices"] = [
            float(p) for p in initial_prices
        ]
        debug_data["calculations"]["final_prices"] = [float(p) for p in final_prices]
        debug_data["calculations"]["il_impact"] = float(il_impact)
        debug_data["calculations"]["final_score"] = float(
            il_impact * avg_correlation * avg_volatility
        )
...
def get_epochs_by_address(pool_id, chain, limit=30, offset=0):
    ...
    try:
        ...
        if not rewards_sugar_address or not rpc_url:
            logger.warning(
                f"Missing RewardsSugar contract address or RPC URL for chain {chain}"
            )
            return None
...
def get_velodrome_pool_sharpe_ratio(
    ...
    try:
        ...
        if not epochs_data:
            logger.warning(
                f"No epochs data available for pool {pool_id}. Cannot calculate Sharpe ratio."
            )
        ...
        for i in range(len(timestamps)):
            fee_return = (
                volumes[i] * pool_fee / total_liquidities[i]
                if total_liquidities[i] > 0
                else 0
            )
        ...
        try:
            ...
            logger.error(
                f"Error combining price and fee returns for pool {pool_id}: {str(e)}"
            )
        ...
        if len(total_rets) > 1:
            try:
                ...
                if total_rets.isna().any() or np.isinf(total_rets).any():
                    logger.warning(
                        f"Pool {pool_id}: NaN or infinite values found in returns data"
                    )
                    ...
                    logger.warning(
                        f"Pool {pool_id}: Found {len(nan_indices)} NaN and {len(inf_indices)} infinite values in returns"
                    )
                ...
                if (
                    len(total_rets) > 3
                ):  # Need enough data points for percentile calculation
                ...
                if returns_std == 0 or np.isnan(returns_std) or np.isinf(returns_std):
                    logger.warning(
                        f"Pool {pool_id}: Standard deviation issue, cannot calculate Sharpe ratio"
                    )
                    return 0  # Return 0 as a default value for constant returns

                if np.isnan(returns_mean) or np.isinf(returns_mean):
                    logger.warning(
                        f"Pool {pool_id}: Mean return issue, cannot calculate Sharpe ratio"
                    )
                ...
                try:
                    ...
                    sharpe_ratio = (excess_returns_mean * 52) / (
                        returns_std * np.sqrt(52)
                    )  # Annualized

                    # Check if result is NaN or infinite
                    if np.isnan(sharpe_ratio) or np.isinf(sharpe_ratio):
                        logger.warning(
                            f"Pool {pool_id}: Sharpe ratio calculation resulted in NaN or infinite value. Mean: {returns_mean}, Std: {returns_std}"
                        )
                        return 0  # Return 0 as a default value
                except Exception as e:
                    logger.error(
                        f"Pool {pool_id}: Error in Sharpe ratio calculation: {str(e)}"
                    )
                    return 0  # Return 0 as a default value
                ...
                logger.info(
                    f"Pool {pool_id}: Successfully calculated Sharpe ratio: {sharpe_ratio}"
                )
                return float(sharpe_ratio)
            except Exception as e:
                logger.error(
                    f"Error calculating Sharpe ratio for pool {pool_id}: {str(e)}"
                )
                return None
        else:
            logger.warning(
                f"Not enough data points for Sharpe ratio calculation for pool {pool_id} (only {len(total_rets)} data points)"
            )
            return None
...
def analyze_velodrome_pool_liquidity(
    pool_id: str,
    chain: str,
    ...
    try:
        ...
        if not epochs_data:
            logger.warning(
                f"No epochs data available for pool {pool_id}. Cannot calculate depth score and max position size."
            )
            return (
                None,
                None,
            )  # Return None to indicate the metrics couldn't be calculated
        ...
        depth_score = (
            (np.log1p(max(0, avg_tvl)) * np.log1p(max(0, avg_vol)))
            / (price_impact * 100)
...
def get_velodrome_pools(
    ...
    if chain_id in SUGAR_CONTRACT_ADDRESSES:
        ...
        return get_velodrome_pools_via_sugar(
            sugar_address, rpc_url=rpc_url, chain_id=chain_id
        )
    else:
...
def get_velodrome_pools_via_sugar(
    ...
    try:
        ...
        for pool in all_pools:
            ...
            formatted_pool = {
                "id": pool["id"],
                "chain": CHAIN_NAMES.get(
                    chain_id, "unknown"
                ),  # Add chain name to pool data
                "inputTokens": [
                    {
                        "id": pool["token0"],
        ...
        logger.info(
            f"Successfully fetched {len(formatted_pools)} pools via Sugar contract"
        )
        ...
        if is_cl_pool:
            ...
            try:
                ...
                if tick_bands:
                    ...
                    logger.warning(
                        f"No tick bands found for CL pool {pool_data.get('id')}, using advertised APR"
                    )
            ...
            return {
                "apr": advertised_apr
            }  # Return basic APR for non-CL pools or if calculation fails
    else:
        return {"apr": 0}
...
def optimize_stablecoin_bands(
    ...
    for current_level, level_config in enumerate(recursion_levels):
        ...
        if (
            ...
            if (
                best_config["band_allocations"][0] > level_config["trigger_threshold"]
...
def calculate_tick_range_from_bands_wrapper(
    ...
    logger.info(f"Band 1 price range: lower={band1_lower}, upper={band1_upper}")

    band2_lower = ema - (band_multipliers[1] * standard_deviation)
    band2_upper = ema + (band_multipliers[1] * standard_deviation)
    logger.info(f"Band 2 price range: lower={band2_lower}, upper={band2_upper}")
    ...
    logger.info(f"Band 1 ticks: lower={band1_tick_lower}, upper={band1_tick_upper}")
    ...
    logger.info(f"Band 2 ticks: lower={band2_tick_lower}, upper={band2_tick_upper}")
    ...
    logger.info(f"Band 3 ticks: lower={band3_tick_lower}, upper={band3_tick_upper}")
...
def get_tick_spacing_velodrome(pool_address: str, chain_id: int) -> Optional[int]:
    ...
    try:
        ...
        contract = web3.eth.contract(
            address=Web3.to_checksum_address(pool_address), abi=VELODROME_CL_POOL_ABI
        )
    ...
    try:
        ...
        if x402_session is not None and x402_proxy is not None:
            ...
            try:
                ...
                response = cg.session.get(f"{cg.api_base_url}{endpoint}", timeout=30)
        ...
        if x402_session is not None and x402_proxy is not None:
            ...
            try:
                ...
                if response:
                    ...
                    timestamps = [
                        entry[0] / 1000 for entry in prices_data
                    ]  # ms to seconds
    ...
    try:
        ...
        try:
            ...
            if pool_data is None:
                logger.error(
                    f"Could not get pool token history for {token0} and {token1}. Aborting operation."
                )
                return None
            ...
            if not ratio_prices:
                logger.error(
                    f"Could not get price ratio history for pool {pool_address}. Aborting operation."
                )
                return None
        except Exception as e:
        ...
        logger.info(f"Collapsed positions before normalization: {collapsed_positions}")
        total_alloc = sum(p["allocation"] for p in collapsed_positions)
        logger.info(f"Total allocation before normalization: {total_alloc}")
        ...
        logger.info(f"Final positions after normalization: {positions}")
        ...
        logger.error(f"Error in stablecoin model calculation: {str(e)}")
        return None
...
def get_filtered_pools_for_velodrome(pools, current_positions, whitelisted_assets):
    ...
    for pool in pools:
        ...
        if token_count >= 2:
            ...
            if whitelisted_tokens:
                ...
                for token in input_tokens:
                    ...
                    if token_address not in whitelisted_tokens:
                        ...
                        logger.debug(
                            f"Pool {pool_id} excluded - token {token_address} not whitelisted"
                        )
                        break
            ...
            logger.debug(
                f"Pool {pool_id} qualified - tokens: {[t['id'] for t in input_tokens]}"
            )
        else:
            logger.debug(
                f"Pool {pool_id} excluded - insufficient tokens ({token_count})"
            )

    logger.info(
        f"Found {len(qualifying_pools)} qualifying pools after initial filtering"
    )
...
def format_velodrome_pool_data(
    ...
    **kwargs,
    ...
    max_allocation_percentage = kwargs.get(
        "max_allocation_percentage", DEFAULT_MAX_ALLOCATION_PERCENTAGE
    )
    ...
    for pool in pools:
        ...
        if position_data is None:
            logger.error(
                f"calculate_position_details_for_velodrome returned None for pool {pool.get('id')}, skipping"
            )
            continue
        ...
        if len(tokens) < 2:
            logger.warning(f"Pool {pool.get('id')} has fewer than 2 tokens, skipping")
            continue
        ...
        if coingecko_api_key or (x402_session and x402_proxy):
            ...
            try:
                ...
                logger.error(
                    f"Error calculating Sharpe ratio for pool {pool['id']}: {str(e)}"
                )
            ...
            try:
                ...
                logger.error(
                    f"Error calculating depth score for pool {pool['id']}: {str(e)}"
                )
            ...
            try:
                ...
                if len(valid_token_ids) >= 2:
                    ...
                    logger.info(
                        f"Pool {pool['id']}: IL risk score calculated: {il_risk_score}"
                    )
                else:
                    logger.warning(
                        f"Pool {pool['id']}: Not enough valid token IDs to calculate IL risk score. Found: {valid_token_ids}"
                    )
                    formatted_pool["il_risk_score"] = None
            except Exception as e:
                logger.error(
                    f"Error calculating IL risk score for pool {pool['id']}: {str(e)}"
                )
                formatted_pool["il_risk_score"] = None
...
def apply_composite_pre_filter(
    ...
    logger.info(
        f"Parameters: top_n={top_n}, apr_weight={apr_weight}, tvl_weight={tvl_weight}, min_tvl_threshold={min_tvl_threshold}, min_volume_threshold={min_volume_threshold}"
    )
    ...
    for pool in pools:
        try:
            ...
            continue
    logger.info(
        f"After TVL filter (>= {min_tvl_threshold}): {len(tvl_filtered_pools)} pools"
    )
    ...
    if min_volume_threshold is not None and min_volume_threshold > 0:
        ...
        for pool in tvl_filtered_pools:
            try:
                ...
                continue
        logger.info(
            f"After volume filter (>= {min_volume_threshold}): {len(volume_filtered_pools)} pools"
        )
    ...
    if cl_filter is not None:
        cl_filtered_pools = [
            pool
            for pool in tvl_filtered_pools
            if pool.get("is_cl_pool", False) == cl_filter
        ]
    ...
    sorted_pools = sorted(
        standardized_pools, key=lambda x: x.get("composite_score", 0), reverse=True
    )
...
def get_top_n_pools_by_apr(pools, n=10, cl_filter=None):
    ...
    if cl_filter is not None:
        filtered_pools = [
            pool for pool in pools if pool.get("is_cl_pool", False) == cl_filter
        ]
    else:
...
def get_opportunities_for_velodrome(
    ...
    **kwargs,
    ...
    cache_key = (
        f"formatted_pools:{chain_id}:{top_n}:{hash(str(sorted(current_positions)))}"
    )
    ...
    # Filter pools
    filtered_pools = get_filtered_pools_for_velodrome(
        pools, current_positions, whitelisted_assets
    )
    ...
    min_volume_threshold = kwargs.get(
        "min_volume_threshold", DEFAULT_MIN_VOLUME_THRESHOLD
    )
    max_allocation_percentage = kwargs.get(
        "max_allocation_percentage", DEFAULT_MAX_ALLOCATION_PERCENTAGE
...
def calculate_metrics(
    ...
    **kwargs,
    ...
    try:
        ...
        # Calculate Sharpe ratio
        sharpe_ratio = get_velodrome_pool_sharpe_ratio(pool_id, chain.upper(), pool_fee)
        ...
        logger.error(
            f"Error calculating metrics for position {position.get('pool_address')}: {str(e)}"
        )
        return None
...
def run(
    ...
    for chain in chains:
        ...
        sugar_address = kwargs.get(
            "lp_sugar_address", SUGAR_CONTRACT_ADDRESSES[chain_id]
        )
        ...
        if isinstance(result, dict) and "error" in result:
            ...
            logger.error(
                f"Error in opportunity discovery for {chain}: {result['error']}"
            )
            continue
        elif not result:
