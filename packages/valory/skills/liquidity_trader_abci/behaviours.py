...
class EvaluateStrategyBehaviour(LiquidityTraderBaseBehaviour):
    ...
    def async_act(self) -> Generator:
        ...
        with self.context.benchmark_tool.measure(self.behaviour_id).local():
            ...
            if self.current_positions:
                dex_type = self.current_positions.get("dex_type")
    ...
    def execute_hyper_strategy(self) -> None:
        ...
        if self.selected_opportunities is not None:
            ...
            if "token0" in self.selected_opportunities:
                self.selected_opportunities["token0"] = to_checksum_address(
                    self.selected_opportunities["token0"]
                )
            if "token1" in self.selected_opportunities:
                self.selected_opportunities["token1"] = to_checksum_address(
                    self.selected_opportunities["token1"]
                )
            ...
            kwargs.update(
                {
                    ...
                    "apr_threshold": self.current_positions.get("apr")
                    if self.current_positions
                    else self.params.apr_threshold,
                    "current_positions": self.current_positions.get("pool_address")
                    if self.current_positions
                    else "",
    ...
    def get_returns_metrics_for_opportunity(self, strategy: str) -> None:
        ...
        if metrics:
            self.current_positions.update(metrics)
            self.context.logger.info(
                f"Updated current pool metrics: {self.current_positions}"
            )
        else:
            self.context.logger.error(
    ...
    def _build_bridge_swap_actions(
        ...
        # Handle STURDY dex type separately
