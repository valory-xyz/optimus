...
class GetPositionsBehaviour(LiquidityTraderBaseBehaviour):
        ...
        for position in self.current_positions:
            ...
            if dex_type == DexType.VELODROME.value and position.get("is_cl_pool"):
                ...
                for pos in position.get("positions", []):
                    if pos.get("current_liquidity", 1) != 0:  # Default to 1 if not found to avoid false closures
                        all_positions_zero = False
                        break
                
                if all_positions_zero and position.get("positions"):  # Only update if there are positions
                ...
                if position.get("current_liquidity", 1) == 0:  # Default to 1 if not found to avoid false closures
                    position["status"] = PositionStatus.CLOSED
                    self.context.logger.info(
    ...
    def update_position_amounts(self) -> Generator[None, None, None]:
        ...
        self.context.logger.info(f"Updating amounts for {len(self.current_positions)} positions")
        ...
        for position in self.current_positions:
            ...
            if not dex_type or not chain:
                self.context.logger.warning(f"Position missing dex_type or chain: {position}")
                continue
                
            self.context.logger.info(f"Updating position of type {dex_type} on chain {chain}")
    ...
    def _update_balancer_position(self, position: Dict[str, Any]) -> Generator[None, None, None]:
        ...
        if not all([pool_address, safe_address, chain]):
            self.context.logger.warning(f"Missing required parameters for Balancer position: {position}")
            return
        ...
        if balance is not None:
            ...
            self.context.logger.warning(f"Failed to get balance for Balancer position: {position}")
    
    def _update_uniswap_position(self, position: Dict[str, Any]) -> Generator[None, None, None]:
        """Update a Uniswap V3 position."""
        token_id = position.get("token_id")
        chain = position.get("chain")
        
        if not all([token_id, chain]):
            self.context.logger.warning(f"Missing required parameters for Uniswap position: {position}")
            return
        ...
        if not position_manager_address:
            self.context.logger.warning(f"No position manager address found for chain {chain}")
            return
        ...
        if position_data and position_data.get("liquidity"):
            ...
            self.context.logger.info(f"Updated Uniswap position liquidity: {position['current_liquidity']}")
        else:
            self.context.logger.warning(f"Failed to get liquidity for Uniswap position: {position}")
    
    def _update_velodrome_position(self, position: Dict[str, Any]) -> Generator[None, None, None]:
        """Update a Velodrome position."""
        chain = position.get("chain")
        is_cl_pool = position.get("is_cl_pool", False)
        
        if not chain:
            self.context.logger.warning(f"Missing required parameters for Velodrome position: {position}")
            return
        ...
        if is_cl_pool:
            ...
            for pos in position.get("positions"):
                ...
                if not token_id:
                    self.context.logger.warning(f"Missing token_id for Velodrome CL position: {pos}")
                    return
                ...
                if position_data and position_data.get("liquidity"):
                    ...
                    self.context.logger.info(f"Updated Uniswap position liquidity: {pos['current_liquidity']}")
                else:
                    self.context.logger.warning(f"Failed to get liquidity for Uniswap position: {pos}")
        else:
            ...
            if not all([pool_address, safe_address]):
                self.context.logger.warning(f"Missing required parameters for Velodrome pool position: {position}")
                return
            ...
            if balance is not None:
                ...
                position["current_liquidity"] = balance
                self.context.logger.info(f"Updated Velodrome pool position amount: {balance}")
            else:
                self.context.logger.warning(f"Failed to get balance for Velodrome pool position: {position}")
    
    def _update_sturdy_position(self, position: Dict[str, Any]) -> Generator[None, None, None]:
        ...
        if not all([pool_address, safe_address, chain]):
            self.context.logger.warning(f"Missing required parameters for Sturdy position: {position}")
            return
        ...
        if balance is not None:
            ...
            self.context.logger.warning(f"Failed to get balance for Sturdy position: {position}")
