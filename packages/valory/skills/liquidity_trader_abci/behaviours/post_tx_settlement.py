...
class PostTxSettlementBehaviour(LiquidityTraderBaseBehaviour):
    ...
    def fetch_and_log_gas_details(self) -> Generator[Any, Any, Any]:
        ...
        if gas_used and effective_gas_price:
            # Normalize to int at the single write chokepoint so the tracker
            # only ever holds ints; downstream readers (e.g. the vanity-tx
            # funding gate) then never hit a hex-string -> ValueError path.
