...
class TestLiquidityTraderBaseBehaviour(LiquidityTraderAbciFSMBehaviourBaseCase):
    ...
    def _create_base_behaviour(self) -> LiquidityTraderBaseBehaviour:
        ...
        return LiquidityTraderBaseBehaviour(
            name="base_behaviour", skill_context=self.skill.skill_context
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                "test_method",
                MagicMock(),
                "Could not successfully interact",
            ),
            ("erc20", "get_balance", MagicMock(), "Could not successfully interact"),
        ],
    )
    def test_default_error(
        self, contract_id, contract_callable, response_msg, expected_log
    ):
    ...
    def test_contract_interaction_error(self, response_body, expected_log_level):
        ...
        with patch.object(
            base_behaviour.context.logger, expected_log_level
        ) as mock_logger:
            base_behaviour.contract_interaction_error(
    ...
    def test_contract_interact_success(
        self,
        performative,
        contract_address,
        contract_public_id,
    ...
    def test_contract_interact_error(
        self,
        performative,
        contract_address,
        contract_public_id,
    ...
    def test_contract_interact_no_data(
        self,
        performative,
        contract_address,
        contract_public_id,
        ...
        [
            (
                ...
                [
                    {
                        "chain": "optimism",
                        "assets": [{"address": "0xToken123", "balance": 1000}],
                    }
                ],
                "0xToken123",
                1000,
            ...
            (
                ...
                [
                    {
                        "chain": "mode",
                        "assets": [{"address": "0xToken456", "balance": 2000}],
                    }
                ],
                "0xToken456",
                2000,
    ...
    def test_store_data_error_handling(self):
        ...
        with patch.object(base_behaviour.context.logger, "error") as mock_error:
            with patch("builtins.open", side_effect=OSError("Permission denied")):
                base_behaviour._store_data(
                    {"test": "data"}, "test_attr", "/invalid/path/file.json"
                )
    ...
    def test_read_data(
        self, attribute, filepath, file_exists, file_content, class_object
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {
                    "enter_timestamp": int(datetime.now().timestamp()) - 86400,
                    "min_hold_days": 0.5,
                },
                True,
            ),
    ...
    @pytest.mark.parametrize(
        "test_name,strategy,strategies_executables,kwargs,expected_result,expected_logs",
        [
            (
                ...
                {
                    "test_strategy": (
                        "def test_method(): return 'success'",
                        "test_method",
                    )
                },
                {},
            ...
            (
                ...
                {
                    "test_strategy": (
                        "def test_method(param): return f'param_{param}'",
                        "test_method",
                    )
                },
                {"param": "value"},
            ...
            (
                ...
                {
                    "test_strategy": (
                        "def test_method(): yield 1; yield 2; yield 3",
                        "test_method",
                    )
                },
                {},
            ...
            (
                ...
                {
                    "test_strategy": (
                        "def test_method(): return {'key': 'value', 'number': 42}",
                        "test_method",
                    )
                },
                {},
    ...
    def test_execute_strategy_comprehensive(
        self,
        test_name,
        strategy,
        ...
        with patch("logging.getLogger") as mock_logger:
            ...
            ), f"Test '{test_name}' failed: expected {expected_result}, got {result}"
    ...
    def test_properties(self):
        ...
        with patch.object(
            type(base_behaviour).__bases__[0],
            "synchronized_data",
            new_callable=PropertyMock,
        ) as mock_super_sync_data:
        ...
        with patch.object(
            type(base_behaviour), "params", new_callable=PropertyMock
        ) as mock_params:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                lambda result: (
                    result is not None
                    and result["action"] == "UnstakeLpTokens"
                    and result["dex_type"] == "velodrome"
                    and result["chain"] == "optimism"
                    and result["is_cl_pool"] is True
                    and result["token_ids"] == [123, 456]
                    and result["gauge_address"] == "0xGauge123"
                ),
            ),
            (
                ...
                lambda result: (
                    ...
                    and result["action"] == "UnstakeLpTokens"
                    and result["dex_type"] == "velodrome"
                    and result["chain"] == "mode"
                    and result["is_cl_pool"] is False
                    and result["gauge_address"] == "0xGauge456"
                ),
            ),
            (
    ...
    def test_gas_cost_tracker(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test log_gas_usage
            tracker.log_gas_usage(
                "optimism", 1234567890, "0xHash123", 21000, 20000000000
            )
    ...
    def test_get_token_decimals(self):
        ...
        with patch.object(base_behaviour, "contract_interact") as mock_contract:
            ...
            result = self._consume_generator(
                base_behaviour._get_token_decimals("optimism", "0xToken123")
            )
            assert result == 18
    ...
    def test_get_token_symbol(self):
        ...
        with patch.object(base_behaviour, "contract_interact") as mock_contract:
            ...
            mock_contract.side_effect = side_effect
            result = self._consume_generator(
                base_behaviour._get_token_symbol("optimism", "0xToken123")
            )
            assert result == "USDC"
    ...
    def test_get_native_balance(self):
        ...
        with patch.object(base_behaviour, "get_ledger_api_response") as mock_ledger:
            ...
            result = self._consume_generator(
                base_behaviour._get_native_balance("optimism", "0xAccount123")
            )
    ...
    def test_get_token_balance(self):
        ...
        with patch.object(base_behaviour, "contract_interact") as mock_contract:
            ...
            mock_contract.side_effect = side_effect
            result = self._consume_generator(
                base_behaviour._get_token_balance(
                    "optimism", "0xAccount123", "0xToken123"
                )
            )
            assert result == 1000000000000000000
    ...
    def test_calculate_min_num_of_safe_tx_required(self):
        ...
        with patch.object(base_behaviour, "_get_liveness_ratio") as mock_liveness_ratio:
            with patch.object(
                base_behaviour, "_get_liveness_period"
            ) as mock_liveness_period:
                with patch.object(
                    base_behaviour, "_get_ts_checkpoint"
                ) as mock_ts_checkpoint:
                    with patch.object(
                        type(base_behaviour),
                        "round_sequence",
                        new_callable=PropertyMock,
                        ...
                        mock_round_sequence.return_value.last_round_transition_timestamp = datetime.fromtimestamp(
                            1000000864
                        )

                        result = self._consume_generator(
                            base_behaviour._calculate_min_num_of_safe_tx_required(
                                "optimism"
                            )
                        )
                        assert result is not None
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "0xToken123",
                "2024-01-01",
                {"token_price_cache_0xtoken123_2024-01-01": '{"2024-01-01": 1.5}'},
                1.5,
            ...
            (
                ...
                None,
                {
                    "token_price_cache_0xtoken123_None": '{"historical": [1.0, 1640995300]}'
                },
                None,
            ...
            (
                "0xToken123",
                None,
                {"token_price_cache_0xtoken123_None": '{"current": [1.0, 1640995300]}'},
                None,
                "Expired current price cache",
            ),
            ...
            (
                ...
                None,
                {
                    "token_price_cache_0xtoken123_None": '{"current": [1.0, "recent_timestamp"]'
                },
                1.0,
                "Valid current price cache",
            ),
            # Test case 6: JSON decode error
            (
                "0xToken123",
                "2024-01-01",
                {"token_price_cache_0xtoken123_2024-01-01": "invalid_json"},
                None,
                "JSON decode error",
            ),
        ],
    ...
    def test_get_cached_price(
        self, token_address, date, read_kv_result, expected_result, test_description
    ):
        ...
        with patch.object(base_behaviour, "_read_kv") as mock_read_kv:
            ...
            if test_description == "Expired current price cache":
                ...
                def expired_side_effect(*args, **kwargs):
                    yield None
                    return {
                        "token_price_cache_0xtoken123_None": f'{{"current": [1.0, {old_timestamp}]}}'
                    }
                ...
                def valid_side_effect(*args, **kwargs):
                    yield None
                    return {
                        "token_price_cache_0xtoken123_None": f'{{"current": [1.0, {recent_timestamp}]}}'
                    }
            ...
            with patch.object(
                ...
                result = self._consume_generator(
                    base_behaviour._get_cached_price(token_address, date)
                )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                2.0,
                "2024-01-01",
                {"token_price_cache_test": '{"2024-01-01": 1.0}'},
                1,
                [],
            ...
            (
                ...
                1.5,
                "2024-01-01",
                {"token_price_cache_test_key_2024-01-01": "invalid_json"},
                1,
                ["Invalid cache data for token test_key, resetting cache"],
                {"2024-01-01": 1.5},
            ),
            ...
            with patch.object(base_behaviour, "_write_kv") as mock_write_kv:
                ...
                with patch.object(base_behaviour.context.logger, "error") as mock_error:
                    self._consume_generator(
                        base_behaviour._cache_price(coin_id, price, date)
                    )
    ...
    def test_get_native_balance_failure(self):
        ...
        with patch.object(base_behaviour, "get_ledger_api_response") as mock_ledger:
            ...
            with patch.object(base_behaviour.context.logger, "error") as mock_error:
                result = self._consume_generator(
                    base_behaviour._get_native_balance("optimism", "0xSafe123")
                )
    ...
    def test_calculate_min_num_of_safe_tx_required_failure_cases(
        self,
        liveness_ratio,
        liveness_period,
        ts_checkpoint,
        ...
        with patch.object(
            type(base_behaviour), "round_sequence", new_callable=PropertyMock
        ) as mock_round_sequence:
            mock_round_sequence.return_value.last_round_transition_timestamp = (
                mock_timestamp
            )

            with patch.object(
                base_behaviour, "_get_liveness_ratio"
            ) as mock_liveness_ratio:
                with patch.object(
                    base_behaviour, "_get_liveness_period"
                ) as mock_liveness_period:
                    with patch.object(
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                True,
                "info",
                "period == 0, forcing immediate update",
                "Period 0 forces immediate update",
            ),
            # Test case 2: Invalid timestamp format
            (
                1,
                {"last_reward_update_optimism": "invalid_timestamp"},
                True,
                "error",
                "Invalid timestamp format in kv_store",
                "Invalid timestamp forces update",
            ),
            ...
            (
                1,
                {"last_reward_update_optimism": "recent_timestamp"},
                False,
                None,
                None,
                "Recent update prevents update",
            ),
            # Test case 5: Old update (older than 12 hours), should update
            (
                1,
                {"last_reward_update_optimism": "old_timestamp"},
                True,
                None,
                None,
                "Old update triggers update",
            ),
        ],
    )
    def test_should_update_rewards_from_subgraph_parameterized(
        ...
        with patch.object(
            type(base_behaviour), "synchronized_data", new_callable=PropertyMock
        ) as mock_sync_data:
            ...
            with patch.object(base_behaviour, "_read_kv") as mock_read_kv:
                ...
                if expected_log_level == "info":
                    with patch.object(
                        ...
                        result = self._consume_generator(
                            base_behaviour.should_update_rewards_from_subgraph(
                                "optimism"
                            )
                        )
                        assert result == expected_result
                    ...
                    with patch.object(
                        base_behaviour.context.logger, "error"
                    ) as mock_logger:
                        result = self._consume_generator(
                            base_behaviour.should_update_rewards_from_subgraph(
                                "optimism"
                    ...
                    result = self._consume_generator(
                        base_behaviour.should_update_rewards_from_subgraph("optimism")
                    )
                    assert result == expected_result
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                0,
                "No rate_limiter attribute",
            ),
            # Test case 3: No credits (line 1394)
            (
                "no_credits",
                {"has_rate_limiter": True, "no_credits": True},
                0,
                "No credits available",
            ),
            # Test case 4: Rate limited with remainder
            (
                "rate_limited_with_remainder",
                {
                    "has_rate_limiter": True,
                    "no_credits": False,
                    "rate_limited": True,
                    "last_request_time": 1000000000,
                    "current_time": 1000000030,
                },
                30,
                "Rate limited with 30s remainder",
            ),
            # Test case 5: Rate limited no remainder
            (
                "rate_limited_no_remainder",
                {
                    "has_rate_limiter": True,
                    "no_credits": False,
                    "rate_limited": True,
                    "last_request_time": 1000000000,
                    "current_time": 1000000060,
                },
                0,
                "Rate limited with no remainder",
            ),
            # Test case 6: Normal operation (no rate limiting)
            (
                "normal",
                {"has_rate_limiter": True, "no_credits": False, "rate_limited": False},
                0,
                "Normal operation",
            ),
        ],
    )
    def test_calculate_rate_limit_wait_time(
        self, test_scenario, coingecko_mock, expected_result, test_description
    ):
        ...
        with patch.object(base_behaviour.context, "coingecko") as mock_coingecko:
            if not coingecko_mock["has_rate_limiter"]:
                ...
                mock_rate_limiter.rate_limited = coingecko_mock.get(
                    "rate_limited", False
                )

                if coingecko_mock.get("last_request_time"):
                    mock_rate_limiter.last_request_time = coingecko_mock[
                        "last_request_time"
                    ]
                ...
                if coingecko_mock.get("current_time"):
                    with patch(
                        "time.time", return_value=coingecko_mock["current_time"]
                    ):
                        result = base_behaviour._calculate_rate_limit_wait_time()
                        assert result == expected_result
                        return
    ...
    def test_get_eth_remaining_amount_comprehensive(
        ...
        with patch.object(base_behaviour, "_read_kv") as mock_read_kv:
            with patch.object(
                base_behaviour, "_get_native_balance"
            ) as mock_native_balance:
                with patch.object(base_behaviour, "_write_kv") as mock_write_kv:
                    with patch.object(
                        base_behaviour, "reset_eth_remaining_amount"
                    ) as mock_reset:
                        with patch.object(
                            base_behaviour.context.logger, "info"
                        ) as mock_info_logger:
                            with patch.object(
                                ...
                                mock_native_balance.side_effect = (
                                    native_balance_side_effect
                                )
                                ...
                                # Execute the method
                                result = self._consume_generator(
                                    base_behaviour.get_eth_remaining_amount()
                                )
                                ...
                                if should_write_kv:
                                    ...
                                    assert write_call_args == {
                                        "eth_remaining_amount": str(on_chain_value)
                                    }
                                else:
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour, "_write_kv") as mock_write_kv:
                with patch.object(base_behaviour.context.logger, "info") as mock_logger:
                    ...
                    # Call the method
                    self._consume_generator(
                        base_behaviour.update_eth_remaining_amount(amount_used)
                    )
                    ...
                    write_call_args = mock_write_kv.call_args[0][0]
                    assert write_call_args == {
                        "eth_remaining_amount": str(expected_new_remaining)
                    }
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour, "_write_kv") as mock_write_kv:
                with patch.object(base_behaviour.context.logger, "info") as mock_logger:
                    ...
                    self._consume_generator(
                        base_behaviour.update_eth_remaining_amount(200)
                    )
    ...
    def test_update_eth_remaining_amount_negative_amount_used(self):
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour, "_write_kv") as mock_write_kv:
                ...
                self._consume_generator(
                    base_behaviour.update_eth_remaining_amount(-100)
                )
    ...
    def test_get_accumulated_rewards_for_token_comprehensive(
        ...
        with patch.object(base_behaviour, "_read_kv") as mock_read_kv:
            with patch.object(base_behaviour.context.logger, "warning") as mock_warning:
                ...
                # Execute the method
                result = self._consume_generator(
                    base_behaviour.get_accumulated_rewards_for_token(
                        chain, token_address
                    )
                )
                ...
                assert (
                    result == expected_result
                ), f"Expected {expected_result}, got {result} for {test_scenario}"
                ...
                if expected_log_message:
                    ...
                    assert (
                        expected_log_message in warning_call
    ...
    def test_get_velodrome_position_principal_comprehensive(
        ...
        with patch.object(
            ...
            mock_params_obj.return_value.velodrome_slipstream_helper_contract_addresses = (
                mock_params
            )
            ...
            if mock_params:
                with patch.object(
                    base_behaviour, "contract_interact"
                ) as mock_contract_obj:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                (0, 0),  # Expected result
                [
                    "No Velodrome Sugar contract address for chain optimism"
                ],  # Error log expected
            ),
            (
                "contract_returns_no_amounts",
    ...
    def test_request_with_retries(
        ...
        with patch.object(
            base_behaviour, "get_http_response", side_effect=mock_get_http_response
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                lambda *args, **kwargs: (yield None)
                or (
                    18 if args[1] == "0xToken0" else None
                ),  # 18 for token0, None for token1
                lambda *args, **kwargs: (yield None) or {},  # Not used
            ...
            (
                ...
                None,
                lambda *args, **kwargs: (yield None)
                or (18 if args[1] == "0xToken0" else 0),  # 18 for token0, 0 for token1
                lambda *args, **kwargs: (yield None) or {},  # Not used
            ...
            (
                ...
                lambda *args, **kwargs: (yield None) or 18,  # Always return 18
                lambda *args, **kwargs: (yield None)
                or {"0xOtherToken": 1000.0},  # Missing token0 price
            ...
            (
                ...
                lambda *args, **kwargs: (yield None) or 18,  # Always return 18
                lambda *args, **kwargs: (yield None)
                or {"0xToken0": 1000.0},  # Missing token1 price
    ...
    def test_calculate_initial_investment_value_comprehensive(
        ...
        with patch.object(
            base_behaviour, "_get_token_decimals", side_effect=decimals_side_effect
        ):
            with patch.object(
                base_behaviour,
                ...
                result = self._consume_generator(
                    base_behaviour.calculate_initial_investment_value(position)
                )

                if expected_result is None:
                    assert (
                        result is None
                    ), f"Test case '{test_case}': {description} - Expected None but got {result}"
                else:
                    assert (
                        result == expected_result
                    ), f"Test case '{test_case}': {description} - Expected {expected_result} but got {result}"
    ...
    def test_calculate_initial_investment(self, current_positions, expected_total):
        ...
        with patch.object(
            ...
            result = self._consume_generator(
                base_behaviour.calculate_initial_investment()
            )
    ...
    def test_calculate_initial_investment_position_key_formation(self):
        ...
        with patch.object(
            ...
            assert (
                base_behaviour.initial_investment_values_per_pool[expected_key]
                == 1000.0
            )
    ...
    def test_query_service_rewards(
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = self._consume_generator(
                    base_behaviour.query_service_rewards(chain)
                )
    ...
    def test_query_service_rewards_json_decode_error(self):
        ...
        with patch.object(
            ...
            mock_params.return_value.staking_subgraph_endpoints = {
                "optimism": "https://api.example.com/subgraph"
            }
            ...
            with patch.object(
                ...
                with pytest.raises(json.JSONDecodeError):
                    self._consume_generator(
                        base_behaviour.query_service_rewards("optimism")
                    )
    ...
    def test_update_accumulated_rewards_for_chain(
        ...
        with patch.object(
            base_behaviour,
            "should_update_rewards_from_subgraph",
            side_effect=mock_should_update,
        ):
            with patch.object(
                base_behaviour,
                "query_service_rewards",
                side_effect=mock_query_service_rewards,
            ):
                with patch.object(
                    base_behaviour, "_write_kv", side_effect=mock_write_kv
                ):
                    with patch.object(
                        base_behaviour,
                        ...
                        self._consume_generator(
                            base_behaviour.update_accumulated_rewards_for_chain(chain)
                        )
                        ...
                        if expected_writes > 0:
                            ...
                            rewards_write = next(
                                (call for call in write_calls if rewards_key in call),
                                None,
                            )
                            ...
                            update_write = next(
                                (call for call in write_calls if update_key in call),
                                None,
                            )
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        self._consume_generator(
                            base_behaviour.update_accumulated_rewards_for_chain(
                                "unknown_chain"
                            )
                        )
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with pytest.raises(AttributeError):
                    self._consume_generator(
                        base_behaviour.query_service_rewards("unknown_chain")
                    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                2,
                [
                    "Fetching SafeApi page: offset=0, limit=100",
                    "Fetching SafeApi page: offset=100, limit=100",
                    "Reached last page of SafeApi results",
                    "Total balances fetched from SafeApi: 3",
                ],
                None,
            ...
            with patch.object(
                ...
                if expected_error:
                    ...
                    with patch.object(
                        ...
                        result = self._consume_generator(
                            base_behaviour._fetch_safe_balances_with_pagination(
                                safe_address
                            )
                        )
    ...
    def test_get_optimism_balances_from_safe_api_parameterized(
        self,
        safe_addresses,
        balances_data,
        expected_result,
        ...
        if safe_addresses:
            ...
            with patch.dict(
                base_behaviour.params.safe_contract_addresses,
                safe_addresses,
                clear=False,
            ):
                if expected_error:
                    ...
                    with patch.object(
                        ...
                        result = self._consume_generator(
                            base_behaviour._get_optimism_balances_from_safe_api()
                        )
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            for i, expected_balance in enumerate(expected_result):
                                assert (
                                    result[i]["asset_symbol"]
                                    == expected_balance["asset_symbol"]
                                )
                                assert (
                                    result[i]["asset_type"]
                                    == expected_balance["asset_type"]
                                )
                                assert (
                                    result[i]["balance"] == expected_balance["balance"]
                                )
                            ...
                            if expected_logs:
                                ...
                                for expected_log in expected_logs:
                                    assert any(
                                        expected_log in call for call in info_calls
                                    )
        else:
    ...
    def test_get_positions_success(self):
        ...
        with patch.object(
            ...
            with patch.object(
                base_behaviour,
                "_get_optimism_balances_from_safe_api",
                side_effect=mock_get_optimism_balances,
            ):
                with patch.object(
                    base_behaviour,
                    "_get_mode_balances_from_explorer_api",
                    side_effect=mock_get_mode_balances,
                ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                3,  # 1 ETH + 2 tokens
                [
                    "Fetching Mode balances from Explorer API for safe: 0xSafe123",
                    "Retrieved 3 token balances from Mode Explorer API",
                ],
            ),
            ...
            (
                ...
                1,  # 1 ETH only
                [
                    "Fetching Mode balances from Explorer API for safe: 0xSafe456",
                    "Retrieved 1 token balances from Mode Explorer API",
                ],
            ),
            ...
            (
                ...
                1,  # 1 token only
                [
                    "Fetching Mode balances from Explorer API for safe: 0xSafe789",
                    "Retrieved 1 token balances from Mode Explorer API",
                ],
            ),
            ...
            (
                ...
                0,  # No balances
                [
                    "Fetching Mode balances from Explorer API for safe: 0xSafeEmpty",
                    "Retrieved 0 token balances from Mode Explorer API",
                ],
            ),
        ],
    )
    def test_get_mode_balances_from_explorer_api(
        ...
        with patch.object(
            ...
            with patch.object(
                base_behaviour,
                "_get_native_balance",
                side_effect=mock_get_native_balance,
            ):
                with patch.object(
                    base_behaviour,
                    "_fetch_mode_token_balances",
                    side_effect=mock_fetch_mode_token_balances,
                ):
                    with patch.object(
                        base_behaviour.context.logger, "info"
                    ) as mock_logger:
                        result = self._consume_generator(
                            base_behaviour._get_mode_balances_from_explorer_api()
                        )
                        ...
                        token_balances_result = [
                            b for b in result if b.get("asset_type") == "erc_20"
                        ]
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour.context.logger, "error") as mock_logger:
                ...
                mock_logger.assert_called_once_with(
                    "No safe address set for Mode chain"
                )
    ...
    def test_fetch_mode_token_balances(
        ...
        with patch.object(
            base_behaviour,
            "_get_active_lp_addresses",
            side_effect=mock_get_active_lp_addresses,
        ):
            with patch.object(
                base_behaviour,
                "_fetch_mode_tokens_with_pagination",
                side_effect=mock_fetch_mode_tokens_with_pagination,
            ):
                with patch.object(
                    base_behaviour.context.logger, "info"
                ) as mock_info_logger:
                    with patch.object(
                        base_behaviour.context.logger, "warning"
                    ) as mock_warning_logger:
                        result = self._consume_generator(
                            base_behaviour._fetch_mode_token_balances(safe_address)
                        )
                        ...
                        # Verify log messages
                        info_log_calls = [
                            call[0][0] for call in mock_info_logger.call_args_list
                        ]
                        warning_log_calls = [
                            call[0][0] for call in mock_warning_logger.call_args_list
                        ]

                        for expected_log in expected_logs:
                            if "Filtering out LP token" in expected_log:
                                assert any(
                                    expected_log in log_call
                                    for log_call in info_log_calls
                                )
                            elif "Invalid balance value" in expected_log:
                                assert any(
                                    expected_log in log_call
                                    for log_call in warning_log_calls
                                )
    ...
    def test_get_liveness_ratio(self, chain, expected_result, should_log_error):
        ...
        with patch.object(base_behaviour, "contract_interact") as mock_contract:
            with patch.object(base_behaviour.context.logger, "error") as mock_logger:
                ...
                result = self._consume_generator(
                    base_behaviour._get_liveness_ratio(chain)
                )
                assert result == expected_result
    ...
    def test_get_liveness_period(self, chain, expected_result, should_log_error):
        ...
        with patch.object(base_behaviour, "contract_interact") as mock_contract:
            with patch.object(base_behaviour.context.logger, "error") as mock_logger:
                ...
                result = self._consume_generator(
                    base_behaviour._get_liveness_period(chain)
                )
                assert result == expected_result
    ...
    def test_is_staking_kpi_met(
        ...
        with patch.object(
            ...
            mock_sync_data.return_value.min_num_of_safe_tx_required = (
                min_num_of_safe_tx_required
            )
            ...
            with patch.object(
                ...
                with patch.object(
                    base_behaviour, "_get_multisig_nonces_since_last_cp"
                ) as mock_get_nonces:
                    ...
                    with patch.object(
                        ...
                        result = self._consume_generator(
                            base_behaviour._is_staking_kpi_met()
                        )
                        assert result == expected_result
    ...
    def test_get_multisig_nonces(
        self, chain, multisig, contract_result, expected_result
    ):
        ...
        with patch.object(base_behaviour, "contract_interact") as mock_contract:
            ...
            result = self._consume_generator(
                base_behaviour._get_multisig_nonces(chain, multisig)
            )
            assert result == expected_result
    ...
    def test_get_multisig_nonces_since_last_cp(
        self, chain, multisig, multisig_nonces, service_info, expected_result
    ):
        ...
        with patch.object(base_behaviour, "_get_multisig_nonces") as mock_get_nonces:
            ...
            with patch.object(
                base_behaviour, "_get_service_info"
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        if service_info is None or (
                            isinstance(service_info, tuple)
                            and (len(service_info) == 0 or len(service_info[2]) == 0)
                        ):
                            mock_logger.assert_called_once()
    ...
    def test_get_service_info(
        self, chain, service_id, contract_result, expected_result
    ):
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour, "contract_interact") as mock_contract:
                with patch.object(
                    ...
                    result = self._consume_generator(
                        base_behaviour._get_service_info(chain)
                    )
                    assert result == expected_result
    ...
    def test_get_service_staking_state(
        self, chain, service_id, contract_result, expected_staking_state
    ):
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour, "contract_interact") as mock_contract:
                with patch.object(
                    ...
                    self._consume_generator(
                        base_behaviour._get_service_staking_state(chain)
                    )
                    assert (
                        base_behaviour.service_staking_state == expected_staking_state
                    )
                    ...
                    with patch.object(base_behaviour, "_cache_price") as mock_cache:
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                result = self._consume_generator(
                                    base_behaviour._fetch_token_price(
                                        token_address, chain
                                    )
                                )
                                assert result == expected_result
    ...
    def test_call_mirrordb(self, method, kwargs, response_payload, expected_result):
        ...
        with patch.object(
            base_behaviour.context, "srr_dialogues"
        ) as mock_srr_dialogues:
            with patch.object(
                ...
                with patch.object(
                    ...
                    result = self._consume_generator(
                        base_behaviour._call_mirrordb(method, **kwargs)
                    )
                    ...
                    assert (
                        call_args[1]["performative"] == SrrMessage.Performative.REQUEST
                    )
                    assert call_args[1]["payload"] == json.dumps(
    ...
    def test_call_mirrordb_exception_handling(self):
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour.context.logger, "error") as mock_logger:
                ...
                result = self._consume_generator(
                    base_behaviour._call_mirrordb("get_data", key="test")
                )

                assert result is None
                mock_logger.assert_called_once_with(
                    "Exception while calling MirrorDB: Connection failed"
                )
    ...
    def test_read_kv(self, keys, response_performative, response_data, expected_result):
        ...
        # Mock context and dialogues
        with patch.object(
            base_behaviour.context, "kv_store_dialogues"
        ) as mock_kv_dialogues:
            with patch.object(
                base_behaviour, "_do_connection_request"
            ) as mock_do_request:
                with patch.object(base_behaviour.context.logger, "info") as mock_logger:
                    ...
                    mock_message = MagicMock()
                    mock_dialogue = MagicMock()
                    mock_kv_dialogues.create.return_value = (
                        mock_message,
                        mock_dialogue,
                    )
        ...
        with patch.object(base_behaviour, "_fetch_token_price") as mock_fetch_token:
            with patch.object(
                ...
                with patch.object(
                    ...
                    result = self._consume_generator(
                        base_behaviour._fetch_token_prices(token_balances)
                    )
                    ...
                    missing_chain_tokens = [
                        t for t in token_balances if not t.get("chain")
                    ]
                    if missing_chain_tokens:
                        for token_data in missing_chain_tokens:
                            mock_logger.assert_any_call(
                                f"Missing chain for token {token_data['token']}"
                            )

                    # Verify method calls
                    token_calls = [
                        call
                        for call in mock_fetch_token.call_args_list
                        if call[0][0] != ZERO_ADDRESS
                    ]
                    zero_calls = [call for call in mock_fetch_zero.call_args_list]

                    expected_token_calls = len(
                        [
                            t
                            for t in token_balances
                            if t.get("chain") and t["token"] != ZERO_ADDRESS
                        ]
                    )
                    expected_zero_calls = len(
                        [t for t in token_balances if t["token"] == ZERO_ADDRESS]
                    )
    ...
    def test_fetch_zero_address_price(
        self, cached_price, api_success, api_response, expected_result
    ):
        ...
        [
            ...
            (
                ...
                {"usd-coin": 1.0, "makerdao-optimism-bridged-dai-optimism": 0.998},
                {
                    ...
                    "0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1": 0.998,
                },
            ),
    ...
    def test_fetch_historical_token_prices(
        self, tokens, date_str, chain, coingecko_ids, historical_prices, expected_result
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = self._consume_generator(
                    base_behaviour._fetch_historical_token_prices(
    ...
    def test_fetch_historical_token_price(
        self,
        coingecko_id,
        date_str,
        cached_price,
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    result = self._consume_generator(
                        base_behaviour._fetch_historical_token_price(
    ...
    def test_fetch_token_name_from_contract(
        self, chain, token_address, contract_result, expected_result
    ):
        ...
        with patch.object(
            base_behaviour, "contract_interact", side_effect=mock_contract_interact
        ):
            result = self._consume_generator(
                base_behaviour._fetch_token_name_from_contract(chain, token_address)
            )
    ...
    def test_fetch_token_name_from_contract_interaction_parameters(self):
        ...
        with patch.object(
            ...
            result = self._consume_generator(
                base_behaviour._fetch_token_name_from_contract(
                    "optimism", "0x0b2C639c533813f4Aa9D7837CAf62653d097Ff85"
                )
            )
    ...
    def test_adjust_current_positions_for_backward_compatibility(
        ...
        with patch.object(
            base_behaviour, "_get_token_symbol", side_effect=mock_get_token_symbol
        ):
            with patch.object(
                base_behaviour,
                "store_current_positions",
                side_effect=mock_store_current_positions,
            ):
                # Call the method
                self._consume_generator(
                    base_behaviour._adjust_current_positions_for_backward_compatibility(
                        input_data
                    )
                )
    ...
    def test_adjust_current_positions_for_backward_compatibility_unexpected_data_format(
        self,
    ):
    ...
    def test_adjust_current_positions_for_backward_compatibility_token_symbol_error_handling(
        self,
    ):
    ...
    def test_adjust_current_positions_for_backward_compatibility_store_current_positions_called(
        self,
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                25.75,
                {
                    "entry_costs_optimism_0xPool123": 10.5,
                    "entry_costs_mode_0xPool789": 15.25,
                },
                2,  # _get_all_entry_costs + _write_kv
            ...
            (
                ...
                {
                    "entry_costs_optimism_0xPool123": 10.5,
                    "entry_costs_mode_0xPool456": 25.75,
                },
                2,  # _get_all_entry_costs + _write_kv
    ...
    def test_store_entry_costs(
        ...
        with patch.object(
            base_behaviour, "_get_all_entry_costs", side_effect=mock_get_all_entry_costs
        ):
            with patch.object(base_behaviour, "_write_kv", side_effect=mock_write_kv):
                with patch.object(
                    ...
                    # Call the method
                    self._consume_generator(
                        base_behaviour._store_entry_costs(chain, position_id, costs)
                    )
        ...
        with patch.object(
            ...
            with patch.object(base_behaviour, "_write_kv", side_effect=mock_write_kv):
                with patch.object(
                    ...
                    self._consume_generator(
                        base_behaviour._store_entry_costs("optimism", "0xPool123", 10.5)
                    )
        ...
        with patch.object(base_behaviour, "_read_kv", side_effect=mock_read_kv):
            with patch.object(base_behaviour, "_write_kv", side_effect=mock_write_kv):
                with patch.object(
                    ...
                    self._consume_generator(
                        base_behaviour._store_entry_costs("optimism", "0xPool1", 10.5)
                    )

                    # Store second entry costs
                    self._consume_generator(
                        base_behaviour._store_entry_costs("mode", "0xPool2", 25.75)
                    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                [
                    "Fetching Mode tokens page:",
                    "Total tokens fetched from Mode Explorer API: 2",
                ],
            ...
            (
                ...
                [
                    {
                        "success": True,
                        "response_data": {"items": [], "next_page_params": None},
                    }
                ],
                [],
                [
                    "Fetching Mode tokens page:",
                    "No more token results from Mode Explorer API",
                    "Total tokens fetched from Mode Explorer API: 0",
                ],
            ...
            (
                "0xSafe4",
                [{"success": False, "response_data": "API Error"}],
                [],
                [
                    "Fetching Mode tokens page:",
                    "Failed to fetch Mode token data: API Error",
                    "Total tokens fetched from Mode Explorer API: 0",
                ],
            ...
            (
                ...
                [
                    ...
                    {"success": False, "response_data": "API Error on second page"},
                ],
                [{"token": "0xToken1", "balance": "1000000000000000000"}],
                [
                    "Fetching Mode tokens page:",
                    "Failed to fetch Mode token data: API Error on second page",
                    "Total tokens fetched from Mode Explorer API: 1",
                ],
                "API failure on second page",
            ),
            ...
            (
                "0xSafe6",
                [{"success": True, "response_data": {"next_page_params": None}}],
                [],
                [
                    "Fetching Mode tokens page:",
                    "No more token results from Mode Explorer API",
            ...
            (
                ...
                [
                    {
                        ...
                        "response_data": {
                            "items": [
                                {
                                    "token": f"0xToken{i}",
                                    "balance": f"{i}000000000000000000",
                                }
                                for i in range(1, 4)
                            ],
                    ...
                    },
                    {
                        ...
                        "response_data": {
                            "items": [
                                {"token": "0xToken7", "balance": "7000000000000000000"}
                            ],
                            "next_page_params": None,
                        },
                    },
                ...
                [
                    {"token": f"0xToken{i}", "balance": f"{i}000000000000000000"}
                    for i in range(1, 8)
                ],
                [
                    "Fetching Mode tokens page:",
                    "Total tokens fetched from Mode Explorer API: 7",
                ],
    ...
    def test_fetch_mode_tokens_with_pagination(
        ...
        def mock_request_with_retries(
            endpoint, method, headers, rate_limited_callback, max_retries, retry_wait
        ):
        ...
        with patch.object(
            ...
            result = self._consume_generator(
                base_behaviour._fetch_mode_tokens_with_pagination(safe_address)
            )
    ...
    def test_fetch_mode_tokens_with_pagination_request_with_retries_exception(self):
        ...
        with patch.object(
            ...
            with pytest.raises(Exception, match="Network error"):
                self._consume_generator(
                    base_behaviour._fetch_mode_tokens_with_pagination("0xSafe8")
                )
    ...
    def test_fetch_mode_tokens_with_pagination_empty_safe_address(self):
        ...
        with patch.object(
            ...
            result = self._consume_generator(
                base_behaviour._fetch_mode_tokens_with_pagination("")
            )
    ...
    def test_fetch_mode_tokens_with_pagination_missing_next_page_params(self):
        ...
        with patch.object(
            ...
            result = self._consume_generator(
                base_behaviour._fetch_mode_tokens_with_pagination("0xSafe9")
            )
    ...
    def test_fetch_mode_tokens_with_pagination_headers_and_parameters(self):
        ...
        with patch.object(
            ...
            self._consume_generator(
                base_behaviour._fetch_mode_tokens_with_pagination("0xSafe10")
            )
            ...
            assert (
                "/api/v2/addresses/0xSafe10/tokens?type=ERC-20" in captured_endpoints[0]
            )
    ...
    def test_fetch_mode_tokens_with_pagination_max_retries_and_wait(self):
        ...
        with patch.object(
            ...
            self._consume_generator(
                base_behaviour._fetch_mode_tokens_with_pagination("0xSafe11")
            )
    ...
    def test_fetch_mode_tokens_with_pagination_rate_limited_callback(self):
        ...
        with patch.object(
            ...
            self._consume_generator(
                base_behaviour._fetch_mode_tokens_with_pagination("0xSafe12")
            )
    ...
    @pytest.mark.parametrize(
        "method_name,method_params,expected_response,expected_call_params",
        [
            (
    ...
    def test_mirrordb_methods(
        self, method_name, method_params, expected_response, expected_call_params
    ):
        ...
        with patch.object(
            base_behaviour, "_call_mirrordb", side_effect=_fake_call_mirrordb
        ) as mock_call:
    ...
    @pytest.mark.parametrize(
        "method_name,method_params,expected_response,expected_call_params,expected_data_structure",
        [
            (
                ...
                },
                {
                    ...
                    "auth": {
                        ...
                        "message": "timestamp:123,endpoint:api/agent-types/123/attributes/",
                    },
                },
            ),
            (
                ...
                },
                {
                    ...
                    "auth": {
                        "agent_id": "agent123",
                        "signature": "0x1234567890abcdef",
                        "message": "timestamp:123,endpoint:api/agents/agent123/attributes/",
                    },
                },
            ),
        ],
    )
    def test_mirrordb_methods_with_signature(
        self,
        method_name,
        method_params,
        expected_response,
        ...
        with patch.object(
            base_behaviour, "sign_message", side_effect=_fake_sign_message
        ) as mock_sign:
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    assert (
                        call_args[1]["method_name"]
                        == expected_call_params["method_name"]
                    )
                    ...
                    expected_agent_id = (
                        method_params[0]
                        if method_name == "create_agent_attribute"
                        else method_params[5]
                    )
    ...
    def test_mirrordb_methods_signature_failure(self, method_name, method_params):
        ...
        with patch.object(
            base_behaviour, "sign_message", side_effect=_mock_sign_message_none
        ) as mock_sign:
            # Mock the round_sequence._last_round_transition_timestamp attribute
            from datetime import datetime
    ...
    @pytest.mark.parametrize(
        "method_name,read_data,get_result,create_result,expected_result,create_params,write_data,exception_message",
        [
            (
                ...
                ),  # create_agent_type params
                {
                    "agent_type": '{"type_id": "123", "type_name": "Modius", "description": "Test description"}'
                },  # _write_kv data
                None,  # No exception
            ),
            (
            ...
            (
                ...
                None,  # get_attr_def_by_name returns None
                {
                    ...
                    "attr_name": "APR",
                    "data_type": "json",
                },  # create_attribute_definition succeeds
                {
                    "attr_def_id": "789",
                    "attr_name": "APR",
                    "data_type": "json",
                ...
                (
                    ...
                    "{}",
                    "agent123",
                ),  # create_attribute_definition params
                {
                    "attr_def": '{"attr_def_id": "789", "attr_name": "APR", "data_type": "json"}'
                },  # _write_kv data
                None,  # No exception
            ),
            (
    ...
    def test_get_or_create_patterns(
        self,
        method_name,
        read_data,
        get_result,
        ...
        with patch.object(
            base_behaviour, "_read_kv", side_effect=_mock_read_kv
        ) as mock_read:
            if method_name == "_get_or_create_agent_type":
                ...
                with patch.object(
                    base_behaviour,
                    "get_agent_type_by_name",
                    side_effect=_mock_get_agent_type_by_name,
                ) as mock_get:
                    if create_result is None and exception_message:
                        ...
                        with patch.object(
                            base_behaviour,
                            "create_agent_type",
                            side_effect=_mock_create_agent_type_fail,
                        ) as mock_create:
                            # Should raise an exception
                            with pytest.raises(Exception, match=exception_message):
                                self._consume_generator(
                                    base_behaviour._get_or_create_agent_type("0x123")
                                )
                    elif create_result:
                        # Test creation case
                        ...
                        with patch.object(
                            base_behaviour,
                            "create_agent_type",
                            side_effect=_mock_create_agent_type,
                        ) as mock_create:
                            # Mock _write_kv
                            ...
                            with patch.object(
                                base_behaviour,
                                "_write_kv",
                                side_effect=_mock_write_kv_agent_type,
                            ) as mock_write:
                                result = self._consume_generator(
                                    base_behaviour._get_or_create_agent_type("0x123")
                                )
                ...
                with patch.object(
                    base_behaviour,
                    "get_attr_def_by_name",
                    side_effect=_mock_get_attr_def_by_name,
                ) as mock_get:
                    if create_result is None and exception_message:
                        # Test failure case
                        ...
                        with patch.object(
                            base_behaviour,
                            "create_attribute_definition",
                            side_effect=_mock_create_attribute_definition_fail,
                        ) as mock_create:
                            # Should raise an exception
                            with pytest.raises(Exception, match=exception_message):
                                self._consume_generator(
                                    base_behaviour._get_or_create_attr_def(
                                        "123", "agent123"
                                    )
                                )
                    elif create_result:
                        ...
                        with patch.object(
                            base_behaviour,
                            "create_attribute_definition",
                            side_effect=_mock_create_attribute_definition,
                        ) as mock_create:
                            # Mock _write_kv
                            ...
                            with patch.object(
                                base_behaviour,
                                "_write_kv",
                                side_effect=_mock_write_kv_attr_def,
                            ) as mock_write:
                                result = self._consume_generator(
                                    base_behaviour._get_or_create_attr_def(
                                        "123", "agent123"
                        ...
                        # Test existing data case
                        result = self._consume_generator(
                            base_behaviour._get_or_create_attr_def("123", "agent123")
                        )
                        assert result == expected_result

    @pytest.mark.parametrize(
        "read_data,agent_type_result,registry_result,create_result,expected_result,write_data,exception",
        [
            (
                {},  # No existing data
                {
                    "type_id": "123",
                    "type_name": "Modius",
                },  # _get_or_create_agent_type succeeds
                None,  # get_agent_registry_by_address returns None
                {
                    "agent_id": "agent123",
                    "agent_name": "TestAgent",
                    "type_id": "123",
                },  # create_agent_registry succeeds
                {
                    "agent_id": "agent123",
                    "agent_name": "TestAgent",
                    "type_id": "123",
                ...
                {
                    "agent_registry": '{"agent_id": "agent123", "agent_name": "TestAgent", "type_id": "123"}'
                },  # _write_kv data
                None,  # No exception
            ),
            (
    ...
    def test_get_or_create_agent_registry_patterns(
        self,
        read_data,
        agent_type_result,
        registry_result,
        ...
        with patch.object(
            base_behaviour,
            "_read_kv",
            side_effect=(exception if exception else _mock_read_kv),
        ) as mock_read:
            if not read_data and not exception:
                ...
                with patch.object(
                    base_behaviour,
                    "_get_or_create_agent_type",
                    side_effect=_mock_get_or_create_agent_type,
                ) as mock_get_type:
                    if agent_type_result:
                        ...
                        with patch.object(
                            base_behaviour,
                            "get_agent_registry_by_address",
                            side_effect=_mock_get_agent_registry_by_address,
                        ) as mock_get_reg:
                            if create_result:
                                # Mock generate_name
                                ...
                                with patch.object(
                                    base_behaviour,
                                    "generate_name",
                                    side_effect=_mock_generate_name,
                                ) as mock_gen_name:
                                    ...
                                    with patch.object(
                                        base_behaviour,
                                        "create_agent_registry",
                                        side_effect=_mock_create_agent_registry,
                                    ) as mock_create:
                                        # Mock _write_kv
                                        ...
                                        with patch.object(
                                            base_behaviour,
                                            "_write_kv",
                                            side_effect=_mock_write_kv,
                                        ) as mock_write:
                                            result = self._consume_generator(
                                                base_behaviour._get_or_create_agent_registry()
                                            )
                                            ...
                                            assert (
                                                call_args[0][0] == "TestAgent"
                                            )  # agent_name (generated)
                                            assert call_args[0][1] == "123"  # type_id
                                            assert (
                                                call_args[0][2]
                                                == base_behaviour.context.agent_address
                                            )  # eth_address
                                ...
                                with patch.object(
                                    ...
                                    def _mock_create_agent_registry_fail(
                                        *args, **kwargs
                                    ):
                                        yield
                                        return None
                                    ...
                                    with patch.object(
                                        base_behaviour,
                                        "create_agent_registry",
                                        side_effect=_mock_create_agent_registry_fail,
                                    ) as mock_create:
                                        result = self._consume_generator(
                                            base_behaviour._get_or_create_agent_registry()
                                        )
    ...
    def test_get_or_create_agent_registry_creation_failure_error_logging(self):
        """Test _get_or_create_agent_registry specifically for error logging when creation fails ."""
        base_behaviour = self._create_base_behaviour()
        ...
        with patch.object(base_behaviour, "_read_kv", side_effect=_mock_read_kv):
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        base_behaviour, "generate_name", side_effect=_mock_generate_name
                    ):
                        # Mock create_agent_registry to FAIL (return None)
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                mock_logger_error.assert_called_once_with(
                                    "Failed to create agent registry"
                                )
    ...
    @pytest.mark.parametrize(
        "address,start_index,syllables,expected_properties",
        [
            (
                "0x1234567890123456789012345678901234567890",
                2,
    ...
    def test_generate_phonetic_name(
        self, address, start_index, syllables, expected_properties
    ):
    ...
    @pytest.mark.parametrize(
        "message,mock_signature,expected_result,expected_encoding",
        [
            (
    ...
    def test_sign_message_success(
        self, message, mock_signature, expected_result, expected_encoding
    ):
        ...
        def _mock_get_signature(*args, **kwargs):
            yield
            return (
                mock_signature.decode("utf-8")
                if isinstance(mock_signature, bytes)
                else mock_signature
            )

        with patch.object(
            base_behaviour, "get_signature", side_effect=_mock_get_signature
        ) as mock_get_sig:
    ...
    @pytest.mark.parametrize(
        "message,mock_signature,expected_result",
        [
    ...
    def test_read_agent_performance_success(self):
        ...
        with patch.object(base_behaviour, "_read_data"):
            ...
            assert (
                base_behaviour.agent_performance["agent_behavior"]
                == "existing_behavior"
            )
    ...
    def test_read_agent_performance_failure(self):
        ...
        with patch.object(
            base_behaviour, "_read_data", side_effect=Exception("Read failed")
        ):
            with patch.object(base_behaviour.context.logger, "warning") as mock_warning:
                base_behaviour.read_agent_performance()
    ...
    def test_store_data_json_dump_error(self):
        ...
        with patch("json.dump", side_effect=OSError("Disk full")):
            with patch.object(base_behaviour.context.logger, "error") as mock_error:
                base_behaviour._store_data(
                    {"test": "data"}, "test_attr", str(file_path)
                )
    ...
    def test_do_connection_request_comprehensive(
        ...
        # Mock the required methods
        with patch.object(
            base_behaviour.context.outbox, "put_message"
        ) as mock_put_message, patch.object(
            base_behaviour, "_get_request_nonce_from_dialogue"
        ) as mock_get_nonce, patch.object(
            base_behaviour, "get_callback_request"
        ) as mock_get_callback, patch.object(
            base_behaviour, "wait_for_message"
        ) as mock_wait:
            ...
            if expected_behavior == "outbox_exception":
                ...
                with pytest.raises(Exception, match="Outbox error"):
                    self._consume_generator(
                        base_behaviour._do_connection_request(
                            message=message, dialogue=dialogue, timeout=timeout
                        )
                    )
                return
    ...
    def test_get_entry_costs_key(self):
        ...
        test_cases = [
            ...
            (
                ...
                "id_with_dashes",
                "entry_costs_chain_with_underscores_id_with_dashes",
            ),
        ]
        ...
        for chain, position_id, expected_key in test_cases:
            ...
            assert (
                result == expected_key
            ), f"Expected {expected_key} for chain={chain}, position_id={position_id}, but got {result}"
    ...
    def test_log_gas_usage_max_records_limit(self):
        ...
        expected_timestamps = [
            int(time.time()) + i for i in range(5, gas_tracker.MAX_RECORDS + 5)
        ]
        ...
        gas_tracker.log_gas_usage(
            chain3, int(time.time()) + 999, "0xEthTxHash999", 999999, 99999999999
        )
        assert len(gas_tracker.data[chain3]) == gas_tracker.MAX_RECORDS
