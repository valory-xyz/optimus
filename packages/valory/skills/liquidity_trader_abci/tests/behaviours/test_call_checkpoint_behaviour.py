...
class TestCallCheckpointBehaviour(LiquidityTraderAbciFSMBehaviourBaseCase):
    ...
    def setup_method(self, **kwargs: Any) -> None:
        ...
        self.checkpoint_behaviour = CallCheckpointBehaviour(
            name="call_checkpoint_behaviour", skill_context=self.skill.skill_context
        )
    ...
    def setup_default_test_data(self):
        ...
        self.checkpoint_behaviour.params.staking_token_contract_address = (
            "0xStakingToken123456789012345678901234567890123456"
        )
        self.checkpoint_behaviour.params.__dict__["_frozen"] = True

    @pytest.mark.parametrize(
        "staking_chain,mock_staking_state,mock_min_tx_required,mock_checkpoint_reached,mock_checkpoint_tx,expected_tx_hex,expected_min_tx,expected_staking_state,test_description",
        [
    ...
    def test_async_act_variations(
        ...
        staking_chain,
        mock_staking_state,
        mock_min_tx_required,
        mock_checkpoint_reached,
        mock_checkpoint_tx,
        expected_tx_hex,
        expected_min_tx,
        ...
        checkpoint_behaviour.params.staking_token_contract_address = (
            "0xStakingToken123456789012345678901234567890123456"
        )
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                assert (
                                    payload.min_num_of_safe_tx_required
                                    == expected_min_tx
                                )
                                assert (
                                    payload.service_staking_state
                                    == expected_staking_state.value
                                )
                                assert payload.chain_id == staking_chain
                                assert (
                                    payload.safe_contract_address
                                    == checkpoint_behaviour.params.safe_contract_addresses.get(
                                        staking_chain
                                    )
                                )
    ...
    def test_check_if_checkpoint_reached_next_checkpoint_none(self):
        ...
        with patch.object(
            ...
            side_effect=mock_get_next_checkpoint,
        ):
    ...
    def test_check_if_checkpoint_reached_timestamp_comparison(
        self, next_checkpoint, timestamp, expected_result, test_description
    ):
    ...
    def test_prepare_checkpoint_tx_success(self):
        ...
        with patch.object(
            checkpoint_behaviour,
            "contract_interact",
            side_effect=mock_contract_interact_checkpoint,
        ):
            with patch.object(
                checkpoint_behaviour,
                "_prepare_safe_tx",
                side_effect=mock_prepare_safe_tx,
            ):
    ...
    def test_prepare_safe_tx_success(self):
        ...
        mock_safe_tx_hash = "0xSafeTxHash1234567890123456789012345678901234567890123456789012345678901234"
        expected_hash_without_prefix = (
            "SafeTxHash1234567890123456789012345678901234567890123456789012345678901234"
        )
        ...
        with patch.object(
            checkpoint_behaviour,
            "contract_interact",
            side_effect=mock_contract_interact_safe,
        ):
            with patch(
                ...
                assert (
                    result
                    == "0xFinalHash1234567890123456789012345678901234567890123456789012345678901234"
                )
    ...
    def test_prepare_safe_tx_empty_hash(self):
        ...
        with patch.object(
            ...
            with pytest.raises(
                ValueError, match="cannot encode safe_tx_hash of non-32 bytes"
            ):
                generator = checkpoint_behaviour._prepare_safe_tx("optimism", mock_data)
    ...
    @pytest.mark.parametrize(
        "staking_chain,safe_addresses,staking_token_address,expected_safe_address,expected_staking_token,test_description",
        [
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    mock_benchmark.measure.assert_called_with(
                        checkpoint_behaviour.behaviour_id
                    )
    ...
    def test_check_if_checkpoint_reached_property_timestamps(self, timestamp):
        ...
        with patch.object(
            checkpoint_behaviour,
            "_get_next_checkpoint",
            side_effect=mock_get_next_checkpoint_earlier,
        ):
            generator = checkpoint_behaviour._check_if_checkpoint_reached("optimism")
    ...
    def test_prepare_checkpoint_tx_contract_interaction_called(self):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                mock_prepare_safe_tx.assert_called_once_with(
                    "optimism", data=mock_checkpoint_data
                )
