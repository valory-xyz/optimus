...
class TestCheckStakingKPIMetBehaviour(FSMBehaviourBaseCase):
    ...
    @pytest.mark.parametrize(
        "is_staking_kpi_met,period_count,period_number_at_last_cp,threshold_period,min_num_of_safe_tx_required,multisig_nonces_since_last_cp,expected_vanity_tx,expected_is_staking_kpi_met,test_description",
        [
            # Test 1: KPI already met
            (True, 10, 5, 5, 5, 10, None, True, "KPI already met"),
            # Test 2: KPI check error (None)
            (None, 10, 5, 5, 5, 10, None, None, "KPI check error"),
            # Test 3: Period threshold not exceeded
            (False, 10, 8, 5, 5, 10, None, False, "period threshold not exceeded"),
    ...
    def test_async_act_variations(
        self,
        is_staking_kpi_met,
        period_count,
        period_number_at_last_cp,
        threshold_period,
        min_num_of_safe_tx_required,
        multisig_nonces_since_last_cp,
        expected_vanity_tx,
        expected_is_staking_kpi_met,
        test_description,
    ):
        ...
        with patch.object(
            check_staking_kpi_behaviour,
            "_is_staking_kpi_met",
            side_effect=mock_is_staking_kpi_met,
        ):
            with patch.object(
                check_staking_kpi_behaviour,
                ...
                with patch.object(
                    check_staking_kpi_behaviour,
                    "_prepare_vanity_tx",
                    side_effect=mock_prepare_vanity_tx,
                ):
                    with patch.object(
                        check_staking_kpi_behaviour, "send_a2a_transaction"
                    ) as mock_send:
                        with patch.object(
                            check_staking_kpi_behaviour, "wait_until_round_end"
                        ) as mock_wait:
                            with patch.object(
                                type(check_staking_kpi_behaviour),
                                "synchronized_data",
    ...
    def test_prepare_vanity_tx_hash_payload_exception(self):
        ...
        with patch.object(
            ...
            with patch(
                ...
                mock_hash_payload.side_effect = Exception(
                    "Hash payload conversion failed"
                )
    ...
    @pytest.mark.parametrize(
        "staking_chain,safe_addresses,expected_safe_address,test_description",
        [
            ...
            ("optimism", {}, None, "missing safe address"),
    ...
    def test_address_handling(
        self, staking_chain, safe_addresses, expected_safe_address, test_description
    ):
    ...
    def test_async_act_benchmark_tool_usage(self):
        ...
        # Mock dependencies
        with patch.object(
            check_staking_kpi_behaviour, "_is_staking_kpi_met"
        ) as mock_is_staking_kpi_met:
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    mock_benchmark.measure.assert_called_with(
                        check_staking_kpi_behaviour.behaviour_id
                    )
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        check_staking_kpi_behaviour, "set_done"
                    ) as mock_set_done:
                        # Execute the actual async_act method
                        list(check_staking_kpi_behaviour.async_act())
    ...
    def test_prepare_vanity_tx_contract_interaction_called(self):
        ...
        with patch.object(
            ...
            "contract_interact",
            side_effect=mock_contract_interact,
        ) as mock_contract_interact_patch:
            with patch(
                "packages.valory.skills.liquidity_trader_abci.behaviours.check_staking_kpi_met.hash_payload_to_hex"
            ) as mock_hash_payload:
                mock_hash_payload.return_value = "0xFinalHash"
    ...
    def test_prepare_vanity_tx_property_chain_names(self, chain_id):
        ...
        with patch.object(
            ...
            with patch(
                ...
                generator = check_staking_kpi_behaviour._prepare_vanity_tx(
                    f"chain_{chain_id}"
                )
                result = self._consume_generator(generator)
        ...
        with patch.object(
            ...
            with patch(
                ...
                mock_logger.info.assert_any_call(
                    "Preparing vanity transaction for chain: optimism"
                )
                mock_logger.debug.assert_any_call(
                    "Safe address for chain optimism: 0xSafeAddress123456789012345678901234567890123456"
                )
                mock_logger.debug.assert_any_call("Transaction data: b'0x'")
                mock_logger.debug.assert_any_call(
                    "Safe transaction hash: 0xSafeTxHash1234567890123456789012345678901234567890123456789012345678901234"
                )
    ...
    def test_prepare_vanity_tx_logging_errors(self):
        ...
        with patch.object(
            check_staking_kpi_behaviour, "contract_interact"
        ) as mock_contract_interact:
            mock_contract_interact.side_effect = Exception(
            ...
            mock_logger.error.assert_called_with(
                "Exception during contract interaction: Contract interaction failed"
            )
    ...
    def test_prepare_vanity_tx_logging_none_hash(self):
        ...
        with patch.object(
            ...
            # Verify error logging
            mock_logger.error.assert_called_with(
                "Error preparing vanity tx: safe_tx_hash is None"
            )
    ...
    def test_prepare_vanity_tx_logging_hash_payload_error(self):
        ...
        with patch.object(
            ...
            with patch(
                ...
                # Verify error logging
                mock_logger.error.assert_called_with(
                    "Exception during hash payload conversion: Hash payload conversion failed"
                )
