...
class TestWithdrawFundsBehaviour(LiquidityTraderAbciFSMBehaviourBaseCase):
    ...
    def setup_method(self, **kwargs: Any) -> None:
        ...
        self.withdraw_behaviour = WithdrawFundsBehaviour(
            name="withdraw_funds_behaviour", skill_context=self.skill.skill_context
        )
    ...
    def test_async_act_no_withdrawal_requested(self):
        ...
        with patch.object(
            ...
            with patch.object(withdraw_behaviour, "send_a2a_transaction") as mock_send:
                with patch.object(
                    withdraw_behaviour, "wait_until_round_end"
                ) as mock_wait:
    ...
    def test_async_act_successful_withdrawal_with_positions(self):
        ...
        with patch.object(
            ...
            with patch.object(
                withdraw_behaviour,
                "_read_withdrawal_data",
                side_effect=mock_read_withdrawal_data,
            ):
                with patch.object(
                    withdraw_behaviour,
                    ...
                    with patch.object(
                        withdraw_behaviour, "send_a2a_transaction"
                    ) as mock_send:
                        with patch.object(
                            withdraw_behaviour, "wait_until_round_end"
                        ) as mock_wait:
    ...
    def test_async_act_no_actions_prepared(self):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    withdraw_behaviour,
                    "_prepare_withdrawal_actions",
                    side_effect=mock_prepare_withdrawal_actions,
                ):
                    with patch.object(
                        withdraw_behaviour,
                        "_update_withdrawal_status",
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    ...
                                    mock_update.assert_any_call(
                                        "WITHDRAWING",
                                        "Withdrawal Initiated. Preparing your funds...",
                                    )
                                    mock_update.assert_any_call(
                                        "FAILED",
                                        "No withdrawal actions could be prepared",
                                    )
    ...
    def test_read_withdrawal_data_success(self):
        ...
        with patch.object(
            withdraw_behaviour, "_read_withdrawal_data", return_value=expected_data
        ):
    ...
    def test_read_withdrawal_data_failure(self):
        ...
        with patch.object(
            withdraw_behaviour, "_read_withdrawal_data", return_value=None
        ):
            result = withdraw_behaviour._read_withdrawal_data()
    ...
    def test_read_investing_paused_true(self):
        ...
        with patch.object(
            withdraw_behaviour, "_read_investing_paused", return_value=True
        ):
    ...
    def test_prepare_withdrawal_actions_complete_flow(self):
        ...
        with patch.object(
            withdraw_behaviour, "_prepare_withdrawal_actions", return_value=mock_actions
        ):
            result = withdraw_behaviour._prepare_withdrawal_actions(
            ...
            assert len(result) == 3
            assert any(
                action.get("action") == Action.EXIT_POOL.value for action in result
            )
            assert any(
                action.get("action") == Action.FIND_BRIDGE_ROUTE.value
                for action in result
            )
            assert any(
                action.get("action") == Action.WITHDRAW.value for action in result
            )
    ...
    def test_prepare_swap_to_usdc_actions_standard(self):
        ...
        with patch.object(
            ...
            with patch.object(
                withdraw_behaviour,
                "_get_usdc_address",
                return_value="0xd988097fb8612cc24eeC14542bC03424c656005f",
            ):
                with patch.object(
                    withdraw_behaviour,
                    "_get_olas_address",
                    return_value="0xcfD1D50ce23C46D3Cf6407487B2F8934e96DC8f9",
                ):
                    result = withdraw_behaviour._prepare_swap_to_usdc_actions_standard(
                        portfolio_data
                    )
    ...
    def test_prepare_transfer_usdc_actions_standard(self):
        ...
        with patch.object(
            withdraw_behaviour, "_get_usdc_address", return_value="0xUSDC"
        ):
            result = withdraw_behaviour._prepare_transfer_usdc_actions_standard(
                target_address
            )
    ...
    def test_update_withdrawal_status_success(self):
        ...
        with patch.object(withdraw_behaviour, "_write_kv", return_value=True):
            list(
                withdraw_behaviour._update_withdrawal_status(
                    "WITHDRAWING", "Test message"
                )
            )
    ...
    def test_update_withdrawal_status_failure(self):
        ...
        withdraw_behaviour = self._create_withdraw_behaviour()
        with patch.object(
            withdraw_behaviour, "_write_kv", side_effect=Exception("KV store error")
        ):
    ...
    @given(
        st.dictionaries(
            st.text(), st.one_of(st.text(), st.integers(), st.floats(), st.booleans())
        )
    )
    def test_prepare_swap_actions_property(self, portfolio_data):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    withdraw_behaviour, "_get_olas_address", return_value="0xOLAS"
                ):
                    result = withdraw_behaviour._prepare_swap_to_usdc_actions_standard(
                        portfolio_data
                    )
    ...
    def test_invalid_position_data(self):
        ...
        with patch.object(
            withdraw_behaviour, "_build_exit_pool_action_base", return_value=None
        ):
            result = withdraw_behaviour._prepare_exit_pool_actions(positions)
    ...
    def test_prepare_withdrawal_actions_with_unstaking_actions(self):
        ...
        with patch.object(
            withdraw_behaviour,
            "_prepare_unstaking_actions",
            return_value=[{"action": "unstake"}],
        ):
            with patch.object(
                withdraw_behaviour,
                "_prepare_exit_pool_actions",
                return_value=[{"action": "exit"}],
            ):
                with patch.object(
                    withdraw_behaviour,
                    "_prepare_swap_to_usdc_actions_standard",
                    return_value=[{"action": "swap"}],
                ):
                    with patch.object(
                        withdraw_behaviour,
                        "_prepare_transfer_usdc_actions_standard",
                        return_value=[{"action": "transfer"}],
                    ):
                        ...
                        assert len(result) == 4
                        assert any(
                            action.get("action") == "unstake" for action in result
                        )
    ...
    def test_prepare_withdrawal_actions_no_transfer_actions(self):
        ...
        with patch.object(
            withdraw_behaviour, "_prepare_unstaking_actions", return_value=[]
        ):
            with patch.object(
                withdraw_behaviour,
                "_prepare_exit_pool_actions",
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                withdraw_behaviour, "_reset_withdrawal_flags"
                            ) as mock_reset:
                                generator = (
                                    withdraw_behaviour._prepare_withdrawal_actions(
                                        positions, portfolio_data, target_address
                                    )
                                )
    ...
    def test_prepare_swap_actions_with_failed_swap_creation(self):
        ...
        with patch.object(
            withdraw_behaviour, "_build_swap_to_usdc_action", return_value=None
        ):
            with patch.object(
                withdraw_behaviour, "_get_usdc_address", return_value="0xUSDC"
            ):
    ...
    def test_update_withdrawal_status_with_specific_exception(self):
        ...
        with patch.object(withdraw_behaviour, "_write_kv", side_effect=mock_write_kv):
            generator = withdraw_behaviour._update_withdrawal_status(
    ...
    def test_prepare_withdrawal_actions_no_open_positions(self):
        ...
        with patch.object(
            ...
            with patch.object(
                withdraw_behaviour, "_prepare_exit_pool_actions", return_value=[]
            ):
                with patch.object(
                    withdraw_behaviour,
                    "_prepare_swap_to_usdc_actions_standard",
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        assert any(
                            action.get("action") == "transfer" for action in result
                        )
    ...
    def test_prepare_exit_pool_actions_with_optional_fields(self):
        ...
        mock_action = {"action": "exit", "pool_address": "0x123"}

        with patch.object(
            withdraw_behaviour, "_build_exit_pool_action_base", return_value=mock_action
        ):
            result = withdraw_behaviour._prepare_exit_pool_actions(positions)
    ...
    def test_prepare_unstaking_actions_with_staking_metadata(self):
        ...
        with patch.object(
            ...
            return_value=mock_unstake_action,
        ):
